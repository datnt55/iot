...
class LocationProviderNMEA extends LocationProvider implements GpsStatus.NmeaListener {
    ...
    // [20190125    VMio] add device accuracy, usually 5 meters
    final float NMEA_DEVICE_ACCURACY = 5.0f; // metter
    ...
    private void updateLocation(String device) {
        // [20190125 VMio] Add accuracy rough estimation from hdop
        // https://stackoverflow.com/questions/6993322/how-do-i-get-the-hdop-or-vdop-values-from-the-gps-locationmanager
        //
        updateLocation(new MLocation(device.equals(TAG), // GPS source is NMEA
                lastFixMillis, latitude, longitude, altitude_gps, alti.climb, vN, vE,
                /*Float.NaN*/Float.isNaN(hdop) ? Float.NaN : hdop * NMEA_DEVICE_ACCURACY, pdop, hdop, vdop, satellitesUsed, satellitesInView
        ), device);
    }
...
//        }
        ...
        try {
            ...
            handleNmea(timestamp, nmea, providerName());
        } catch(Exception e) {
            Exceptions.report(new NMEAException("Exception while handling NMEA: " + nmea, e));
        }
    }

    protected void handleNmea(long timestamp, String nmea, String device) throws NMEAException {
        // Parse NMEA command
        ...
        switch (command) {
            ...
            case "RMC":
                ...
                // Log.("Time", "["+timestamp+"] lastFixMillis = " + lastFixMillis + ", currentTime = " + System.currentTimeMillis());
